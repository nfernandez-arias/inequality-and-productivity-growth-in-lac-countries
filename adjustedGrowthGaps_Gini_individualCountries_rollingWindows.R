...
computeAdjustments_LACvsNonLAC <- function(data, tempMetric) {
  ...
  modelGinis <- plm(get(tempMetric) ~ lcgdpoPerCapita_gap_avg7 + isLAC + gini_disp_avg7, data, index = c("alpha3","year"), effect = "time", model = "within")
  ...
  tempGinis <- data[ , .(adjustmentGinisOutputGap = mean(na.omit(lcgdpoPerCapita_gap_avg7 * modelGinis$coefficients[1])), 
                         adjustmentGinisGiniDisp = mean(na.omit(gini_disp_avg7 * modelGinis$coefficients[3]))), by = .(countryLAC,year)]
  ...
  tempGinisNonLAC <- tempGinis[countryLAC == "nonLAC"][ , .(year,adjustmentGinisOutputGap, adjustmentGinisGiniDisp)]
  ...
  tempGinis <- tempGinis[ , .(countryLAC,year,adjustmentGinisOutputGap,adjustmentGinisGiniDisp)]
...
for (metric in c("lrtfpna_chg7","lrgdpnaPerCapita_chg7_nonTFP","lrgdpnaPerCapita_chg7")) 
{
  ...
  get(tempStringRaw)[ , gap_adjustedGinis := gap - adjustmentGinisOutputGap - adjustmentGinisGiniDisp]
    
  assign(tempStringRaw, get(tempStringRaw)[ , .(countryLAC, year, gap, gap_adjusted, gap_adjustedRobust, gap_adjustedGinis, adjustment, adjustmentGinisOutputGap, adjustmentGinisGiniDisp)])
  
  assign(tempStringRaw, melt(get(tempStringRaw), id.vars = c("countryLAC","year"),
                                                  measure.vars = c("gap","gap_adjusted","gap_adjustedRobust","gap_adjustedGinis","adjustment","adjustmentGinisOutputGap","adjustmentGinisGiniDisp")))
...
#temp <- LACvsNonLAC_lrtfpna_chg7[variable %in% c("adjustmentGinisOutputGap","adjustmentGinisGiniDisp")]

########
#### Figure 14
########

ggplot(LACvsNonLAC_lrtfpna_chg7[variable %in% c("adjustmentGinisOutputGap","adjustmentGinisGiniDisp") & countryLAC != "BOL" & countryLAC != "VEN"], aes(x = year-3, y = -value, color = variable)) + 
  geom_line() +
  #geom_line( aes(y = gap_adjustedRobust, linetype = "gap_adjustedRobust")) + 
  #geom_line( aes(y = gap_adjustedGinis, linetype = "gap_adjustedGinis")) + 
  facet_wrap(~countryLAC) + 
  labs(title = "Adjustments for output per capita gap (rel. USA) and Gini index vs. non-LAC ",
       subtitle = "LAC countries, productivity") + 
  ylab("Annual growth rate (%)") +
  xlab("Year") + 
  scale_color_discrete(name = "", labels = c("Output per capita (rel. USA)","Gini coefficient")) + 
  theme(legend.position = "bottom") + 
  ggsave("figures/finalFiguresAndTables/Figure14.pdf", width = 12, height = 9, units = "in")
